    ...
    >
      ...
      <div className="relative h-[260px] overflow-hidden">
        ...
        <div className="absolute top-4 left-4 flex flex-wrap gap-2">
          <div className="px-4 py-2 rounded-2xl bg-white/90 backdrop-blur-md border border-white/20 shadow-sm flex items-center gap-2">
            ...
            <span className="text-[12px] font-bold text-slate-900">{pkg.duration}</span>
          </div>
        </div>
        <div className="absolute bottom-1 left-6 px-4 py-2 rounded-2xl bg-slate-900/90 backdrop-blur-lg border border-white/10 text-white">
          ...
          <span className="text-[10px] opacity-70 ml-1 uppercase tracking-tighter">/ person</span>
        </div>
      </div>
      ...
      <div className="relative flex flex-col justify-between p-6 flex-grow">
        <div>
          ...
          {/* 4 Highlights instead of 2 */}
          <div className="flex flex-wrap gap-2 mb-4">
            {pkg.highlights.slice(0, 4).map((h, i) => (
              <span key={i} className="text-[10px] px-2.5 py-1 bg-slate-100/80 text-slate-600 rounded-lg border border-slate-200/50 font-medium">
                {h}
              </span>
            ))}
          </div>
        ...
        <motion.div {...buttonZoomProps}>
          ...
          >
            <span className="relative z-10 font-bold text-sm tracking-wide">Explore Experience</span>
    ...
    <section className="relative py-29 md:py-33 px-5 bg-[#F8F9FA] overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-7 items-end mb-16">
          ...
          <motion.p initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="text-slate-500 text-lg md:text-xl max-w-md leading-relaxed border-l-2 border-sky-200 pl-6">
            Discover Kanthalloor through routes that locals love. Hand-picked stays and hidden spots.
          </motion.p>
        </div>
        ...
        {/* Bespoke CTA Section */}
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-22">
          <div className="relative bg-slate-950 p-10 md:p-15 rounded-[50px] flex flex-col lg:flex-row items-center justify-between gap-12 overflow-hidden">
            <div className="relative z-10 max-w-xl">
              <h3 className="text-3xl md:text-5xl font-bold text-white mb-6 tracking-tight">Need a custom plan?</h3>
              <p className="text-slate-400 text-lg">Tell us your group size and dates, and we'll craft an itinerary just for you.</p>
            </div>

            {/* Gradient Zoom Button */}
            <motion.div {...buttonZoomProps} className="inline-block relative z-10">
              ...
              >
                <div className="text-left">
                  <span className="block text-[9px] uppercase tracking-[0.2em] opacity-80 mb-1 leading-none">Instant Support</span>
